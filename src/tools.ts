...
import { identifyBy, type VaultService } from './service.js';
...
function listBriefish(files: unknown[]): string {
  if (!files.length) return '(空目录)';
...
/** 注册全部 obsidian_* 工具。 */
export function registerTools(ctx: Context, svc: VaultService): void {
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render(args: unknown, v: any) {
        if (!v.ok) return text(['❌ Obsidian 端点探测失败' + BASE_ERR + v.error]);
        return text([
          '✅ Obsidian REST 端点可用',
          '服务: ' + (v.service || '-'),
    ...
    async execute(_args: unknown, _exec: unknown) {
      return (await svc.health()) as any;
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render(_args: unknown, v: any) {
        if (!v.ok) return text(['❌ 目录列表失败' + BASE_ERR + v.error]);
    ...
    async execute(args: any) {
      ...
      return (await svc.listDir(p.replace(/^\/+/, ''))) as any;
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render(_args: unknown, v: any) {
        if (!v.ok) return text(['❌ 读取失败' + BASE_ERR + v.error]);
        return text(['📄 ' + v.path + '（' + v.content.length + ' 字符）', v.content]);
      },
    },
    timeoutMs: 30000,
    async execute(args: any) {
      return (await svc.readNote(String(args.path))) as any;
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render(_args: unknown, v: any) {
        if (!v.ok) return text(['❌ 搜索失败' + BASE_ERR + v.error]);
    ...
    timeoutMs: 30000,
    async execute(args: any) {
      return (await svc.search(String(args.query || ''), args.tag ? String(args.tag) : undefined, args.dir ? String(args.dir) : undefined)) as any;
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render(_args: unknown, v: any) {
        if (!v.ok) return text(['❌ 创建失败' + BASE_ERR + v.error]);
        return text(['✅ 已创建笔记 ' + v.path]);
      },
    },
    timeoutMs: 30000,
    async execute(args: any, exec: any) {
      return (await svc.createNote(String(args.path), String(args.content), args.frontmatter ?? null, identifyBy(exec))) as any;
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render(_args: unknown, v: any) {
        if (!v.ok) return text(['❌ 更新失败' + BASE_ERR + v.error]);
        return text(['✅ 已覆盖写入 ' + v.path]);
      },
    },
    timeoutMs: 30000,
    async execute(args: any, exec: any) {
      return (await svc.updateNote(String(args.path), String(args.content), identifyBy(exec))) as any;
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render(_args: unknown, v: any) {
        if (!v.ok) return text(['❌ 删除失败' + BASE_ERR + v.error]);
        return text(['🗑️ 已永久删除 ' + v.path + '（不可恢复）']);
      },
    },
    timeoutMs: 30000,
    async execute(args: any, exec: any) {
      return (await svc.deleteNote(String(args.path), identifyBy(exec))) as any;
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render(_args: unknown, v: any) {
        if (!v.ok) return text(['❌ 命令列表失败' + BASE_ERR + v.error]);
        return text(['⚙️ 可用命令 ' + v.commands.length + ' 个', JSON.stringify(v.commands)]);
      },
    },
    timeoutMs: 30000,
    async execute() {
      return (await svc.listCommands()) as any;
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render(_args: unknown, v: any) {
        if (!v.ok) return text(['❌ 命令执行失败' + BASE_ERR + v.error]);
        return text(['✅ 已执行命令 ' + v.commandId, JSON.stringify(v.result)]);
      },
    },
    timeoutMs: 60000,
    async execute(args: any, exec: any) {
      return (await svc.runCommand(String(args.commandId), args.args ?? null, identifyBy(exec))) as any;
    },
  }));
}