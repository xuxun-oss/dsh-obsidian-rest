...
export function apply(ctx: Context, config: unknown = {}): void {
  ...
  ctx.on('tools/pre-execute', (exec: any, next: any) => {
    const decision = preExecuteDecision(exec.name, exec.arguments as Record<string, unknown> | undefined, cfg);
    if (!decision.owned) return next();
