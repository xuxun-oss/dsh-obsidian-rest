...
export function resolveConfig(raw: unknown): ResolvedConfig {
  ...
  try {
    cfg = Config(raw == null ? {} : raw) as unknown as ResolvedConfig;
  } catch (e) {
  ...
  if (!apiKey) apiKey = String(cfg.apiKey || '').trim();

  const errors: string[] = [];
  if (!baseUrl) {
    errors.push(
      '缺少 baseUrl（请填写你自己的 Obsidian Local REST API 地址，如 https://vault.example.com，参考 Obsidian 社区插件 Local REST API 的设置）',
    );
  }
  if (!apiKey) {
    errors.push(
