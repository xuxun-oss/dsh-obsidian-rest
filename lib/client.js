...
function netError(err) {
    ...
    if (e && e.code === 'ENOTFOUND')
        return new ObsidianError('网络错误：无法解析 baseUrl 域名' + code);
...
function httpError(status, data) {
    const m = data && typeof data === 'object' && data.message
        ? String(data.message) : '';
    const tail = m ? ' — ' + m : '';
    if (status === 401)
        return new ObsidianError('认证失败（HTTP 401）：API key 无效或缺失' + tail, status);
    if (status === 403)
        return new ObsidianError('访问被拒绝（HTTP 403）：被 Cloudflare Access 或服务端拦截' + tail, status);
    if (status === 404)
        return new ObsidianError('路径或端点不存在（HTTP 404）' + tail, status);
    if (status >= 500)
        return new ObsidianError('Obsidian 服务端错误（HTTP ' + status + '）' + tail, status);
...
export class ObsidianClient {
    ...
    request(method, urlPath, body) {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                reject(new ObsidianError('baseUrl 非法：' + String(e.message)));
                return;
            }
            ...
            if (isHttps)
                opts.rejectUnauthorized = this.cfg.rejectUnauthorized;
    ...
    async requestOk(method, urlPath, body) {
    ...
    async health() {
        ...
        this.manifest = (res.data && typeof res.data === 'object') ? res.data : {};
    ...
    async listDir(path) {
        ...
        const files = res.data && typeof res.data === 'object' && Array.isArray(res.data.files)
            ? res.data.files : [];
        return files;
    }
    ...
    async listCommands() {
        ...
        const commands = res.data && typeof res.data === 'object' && Array.isArray(res.data.commands)
            ? res.data.commands : [];
        return commands;
    }
