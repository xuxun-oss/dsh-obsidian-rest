...
export function normalizeVaultPath(input) {
    if (typeof input !== 'string')
        return fail('路径必须是非空字符串');
    if (input.length === 0 || input.trim() === '')
        return fail('路径为空');
    if (input.includes('\0'))
        return fail('路径含非法字符');
    if (WINDOWS_DRIVE.test(input))
        return fail('不允许绝对路径或盘符');
    if (IS_ABSOLUTE.test(input))
        return fail('不允许绝对路径（vault 相对路径必须以目录/文件名开头）');
    // 统一分隔符：反斜杠转正斜杠。
    ...
    for (const seg of p.split('/')) {
        ...
        if (seg === '..') {
            if (stack.length === 0)
                return fail('路径越出 vault 根（`..` 逃逸被拒绝）');
            stack.pop();
        }
        else {
            stack.push(seg);
 ...
 */
export function hasHiddenSegment(normalized) {
    return normalized.split('/').some((s) => s.startsWith('.'));
}
/** 写操作护栏：返回错误信息，无错误返回空串。 */
export function writeProtectionReason(normalized) {
...
export function validateWritablePath(input) {
    const r = normalizeVaultPath(input);
    if (!r.ok)
        return r;
 ...
 */
export function encodeVaultPath(normalized) {
